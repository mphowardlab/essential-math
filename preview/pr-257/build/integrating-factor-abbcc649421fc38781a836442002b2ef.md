...
```{exercise}
:label: integrating-factor-sb-1
$\displaystyle y' = \frac{x^4 + y^2}{xy}$
```

```{solution} integrating-factor-sb-1
:class: dropdown
...
```{exercise}
:label: integrating-factor-sb-2
$\displaystyle y' = -\frac{e^{x+y} + ye^y}{x e^y - 1}$
```

```{solution} integrating-factor-sb-2
:class: dropdown
This ODE is not in the standard form, so we need to first rearrange:
...
```{exercise}
:label: integrating-factor-sb-3
$\displaystyle y' = y + 1 - 2x$
```

```{solution} integrating-factor-sb-3
:class: dropdown
