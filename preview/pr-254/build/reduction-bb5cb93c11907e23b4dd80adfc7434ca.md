...
```{exercise}
:label: second-order-odes-reduction-sb-1
$xy'' + 2y' + xy = 0$, given $y_1 = \cos x / x$
```

```{solution} second-order-odes-reduction-sb-1
:class: dropdown
...
```{exercise}
:label: second-order-odes-reduction-sb-2
$(1-x^2)y'' - 2xy' + 2y = 0$
```

```{solution} second-order-odes-reduction-sb-2
:class: dropdown
