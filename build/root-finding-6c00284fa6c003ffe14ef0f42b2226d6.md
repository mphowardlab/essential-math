...
Note that the choice of rearranging $f(x)$ is not unique, and some choices may
...
```{warning}
This method may still fail spectacularly! You can try to improve
...
```{exercise}
:label: root-finding-sb-1
Fixed-point iteration starting from $x_0 = 0.5$.
```

```{solution} root-finding-sb-1
:class: dropdown
Rearrange as
...
```{exercise}
:label: root-finding-sb-2
Bisection search in $0 \le x \le 1$.
```

```{solution} root-finding-sb-2
:class: dropdown
| $n$    | $a_n$  | $b_n$  | $x_n$  | $f(a_n)$| $f(b_n)$| $f(x_n)$|
|--------|---------|---------|---------|---------|---------|---------|
...
```{exercise}
:label: root-finding-sb-3
Newton-Raphson method, starting from $x_0 = \pi/2$.
```

```{solution} root-finding-sb-3
:class: dropdown
For
...
|-----|--------|-------------------------|-----------|
| 0   | 1.571  |  -1.571                 | -2.0      |
| 1   | 0.7854 | $-7.829 \times 10^{-2}$ | -1.7071   |
| 2   | 0.7395 | $-7.549 \times 10^{-4}$ | -1.674    |
| 3   | 0.7391 | $-7.513 \times 10^{-8}$ | -1.674    |
| 4   | 0.7391 |                         |           |
